...
function FeedContent() {
  ...
  useEffect(() => {
    return () => {
      ...
      if (window.globeInstance) window.globeInstance = null;
    };
  }, [currentIndex]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      if (session?.user) { fetchUserLikes(session.user.id); } else { setUserLikes(new Set()); }
    });
  ...
  const fetchUserLikes = async (userId: string) => {
    const { data, error } = await supabase.from('interactions').select('post_id').eq('user_id', userId).eq('type', 'like');
    if (!error && data) { setUserLikes(new Set(data.map(i => i.post_id))); }
  };
  ...
  useEffect(() => {
    ...
    async function fetchPosts() {
      const { data, error } = await supabase.from('posts').select(`*, categories (id, name, slug)`).in('type', ['custom', 'cesium', 'globe', 'leaflet']).order('created_at', { ascending: false });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { e.preventDefault(); navigateToPost(currentIndex + 1); } 
      else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { e.preventDefault(); navigateToPost(currentIndex - 1); }
    };
  ...
  const navigateToPost = (index: number) => {
    ...
    setTimeout(() => {
      ...
      containerRef.current?.children[index]?.scrollIntoView({ behavior: 'smooth', inline: 'start', block: 'nearest' });
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        ...
        setUserLikes(prev => { const newSet = new Set(prev); newSet.delete(post.id); return newSet; });
        setPosts(prev => prev.map((p, i) => i === currentIndex ? { ...p, likes_count: Math.max(0, (p.likes_count || 0) - 1) } : p));
      } else {
        ...
        setPosts(prev => prev.map((p, i) => i === currentIndex ? { ...p, likes_count: (p.likes_count || 0) + 1 } : p));
      }
    } catch (error) { console.error('Error toggling like:', error); }
  };

  const handleCommentClick = () => { if (!user) { window.location.href = '/home'; return; } setIsCommentsOpen(true); };
